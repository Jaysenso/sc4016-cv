...
for doc_idx = 1:4
    ...
    objective_func = @(params)sauvola_objective_scalar(img, ground_truth, params.window_size, params.k, params.R);
    ...
    [final_error, binary_img, diff_img, error_percentile] = ...
        sauvola_objective(img, ground_truth, optimal_window, optimal_k, optimal_R);
    ...
    all_results{doc_idx}.optimal_R = optimal_R;
...
fprintf('%-8s %-12s %-12s %-12s %-15s %-15s\n', 'Doc', 'Window', 'k', 'R', 'Error', 'Error (%)');
fprintf('%s\n', repmat('-', 1, 85));

for doc_idx = 1:4
    result = all_results{doc_idx};
    fprintf('%-8d %-12d %-12.4f %-12.4f %-15d %-15.2f\n', ...
        result.doc_idx, result.optimal_window, result.optimal_k, result.optimal_R, ...
    ...
    figure('Name', sprintf('Document %d - Sauvola Optimization Results', doc_idx), ...
    ...
    % Optimized Sauvola result
    subplot(2, 2, 2);
    imshow(result.binary_img);
    title(sprintf('Optimized Sauvola\nW=%d, k=%.4f, R=%.2f', ...
        result.optimal_window, result.optimal_k, result.optimal_R), ...
    ...
    sgtitle(sprintf('Document %d: Sauvola Optimization Results', doc_idx), ...
            'FontSize', 14, 'FontWeight', 'bold');
end

%% Scalar objective function for Bayesian optimization (returns only error)
function error = sauvola_objective_scalar(img, ground_truth, window_size, k, R)
    [error, ~, ~, ~] = sauvola_objective(img, ground_truth, window_size, k, R);
end

%% Full objective function for Sauvola thresholding (returns all outputs)
function [error, binary_img, diff_img, error_percentile] = sauvola_objective(img, ground_truth, window_size, k, R)
